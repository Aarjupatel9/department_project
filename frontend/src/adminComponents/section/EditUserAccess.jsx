...
export default function EditUserAccess() {
  ...
  const verify = () => {
    ...
    const verifyPromise = adminService.verifyUserAccount(id);
    verifyPromise.then((res) => {
      setUserBasicDetail((prev) => ({ ...prev, isVerified: true }));
    });
    toast.promise(
      verifyPromise,
      {
  ...
  const approve = () => {
    ...
    const verifyPromise = adminService.approveUserAccount(id);
    verifyPromise.then((res) => {
      setUserBasicDetail((prev) => ({ ...prev, isApproved: true }));
    });
    toast.promise(
      verifyPromise,
      {
  ...
  const handleRoleChange = (event) => {
    ...
    roleUpdatePromise.then((res) => {
      setUserBasicDetail((prev) => ({ ...prev, role: value }));
    });
    toast.promise(
      roleUpdatePromise,
      {
