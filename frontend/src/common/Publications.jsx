...
const Publication = () => {
  ...
  useEffect(() => {
    const eventPromise = publicationService.getPublications();
    toast.promise(
      eventPromise,
      {
        loading: "fetching Publication details",
        success: "",
        error: (err) => err,
      },
      {
  ...
  function setFilterList(input) {
    const tmp = publications.filter((publication) => {
      return publication;
    });
  ...
  const handelEventDelete = (_id) => {
    const eventPromise = publicationService.deletePublication(_id);
    eventPromise
      .then((res) => {
        console.log("users : ", res);
        const tmp = publications.filter((publication) => {
          if (publication._id != _id) {
            return publication;
          }
        });
        setFilteredEvents(tmp);
    ...
    toast.promise(
      eventPromise,
      {
        loading: "please wait while we deleting publication",
      ...
      {filteredEvents.length > 0 ? (
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 bg-gray-100 dark:bg-gray-800">
                        {formatDateToDdMmYyyy(publication.publicationDate.toString())}
                      </td>
                      <td className="px-6 py-4 dark:text-white dark:bg-gray-700">
                        <ul>
                      ...
                      <td className="px-6 py-4 text-right dark:text-white bg-gray-100 dark:bg-gray-800">
                        <div
                          onClick={() => {
                            navigate("/editPublication/" + publication._id);
                          }}
                      ...
                      <td className="px-6 py-4 text-right dark:text-white dark:bg-gray-700">
                        <div
                          onClick={() => {
                            handelEventDelete(publication._id);
                          }}
        ...
        <div className="flex flex-col pt-4">
          <h1 className="mx-auto  text-2xl font-bold text-gray-900 dark:text-white">
            There is no publication Details
          </h1>
        </div>
      )}
    </div>
